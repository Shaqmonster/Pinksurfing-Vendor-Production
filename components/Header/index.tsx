...
const Header = (props: { loggedIn: boolean | undefined }) => {
  ...
  const { setAuthpage, authPage, setSidebarOpen, sidebarOpen, refreshAuth, authReady } =
    useContext(MyContext);
  const [profile, setProfile] = useState<any>();
  const [Logged, setLogged] = useState(false);
  const [tokens, setTokens] = useState({
    access: "" as string | null,
    vendor_id: "" as string | null,
    refresh: "" as string | null,
  });
  ...
  useEffect(() => {
    ...
    const syncSession = async () => {
      ...
      if (access || props.loggedIn) {
        setLogged(true);
        setTokens({
          access: access || getAccessToken(),
          vendor_id: localStorage.getItem("vendor_id"),
          refresh: getRefreshToken(),
        });

        if (access && !localStorage.getItem("vendor_id")) {
    ...
    void syncSession();
  }, [pathname, router, props.loggedIn, authReady, refreshAuth]);
  ...
  }, [tokens.access, tokens.vendor_id, props.loggedIn, refreshAuth]);
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="hidden md:block">
                  ...
                  <p className="text-sm text-surface-500 dark:text-surface-400">
                    {profile?.store_name
                      ? `Welcome back, ${profile.contact_person_name || profile.store_name}`
                      : "Manage your store"}
                  </p>
                </div>
              </div>
